...
if(isset($_GET['id'])){
    $update_id = $_GET['id'];
    $sql = mysqli_query($con,"SELECT * FROM `slider` WHERE id='$update_id'");
...
if(isset($_POST['submitt'])){
    ...
    if($_FILES['Logo']['name']!=''){
        ...
        $sql =  mysqli_query($con,"UPDATE `slider` SET `link`='$link' ,`image`='$destinationfile'  WHERE id='$ban_id'");
    }
    else{
        $destinationfile ='../'. $destinationfile;
        $sql =  mysqli_query($con,"UPDATE `slider` SET `link`='$link'   WHERE id='$ban_id'");
    }
